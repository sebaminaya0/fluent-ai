...
class BlackHoleReproductionThread(threading.Thread):
    ...
    def __init__(
        self, output_device: int, input_queue: queue.Queue, sample_rate: int = 44100
    ):
        super().__init__()
    ...
    def run(self):
        ...
        logger = logging.getLogger(__name__)
        logger.info(
            f"Starting BlackHole playback thread on device {self.output_device}"
        )
            ...
            with sd.OutputStream(
                ...
                while not self.stop_event.is_set():
                    try:
                        ...
                        logger.info(
                            f"Got audio chunk from queue: {len(audio_chunk)} samples"
                        )
                        ...
                        try:
                            ...
                            logger.info(
                                f"Played audio chunk: {len(audio_chunk)} samples"
                            )
    ...
    def list_output_devices() -> list[dict]:
        ...
        Each entry: ``{'index': int, 'name': str, 'is_blackhole': bool}``
        """
        devices = []
