...
class TestSilenceDetectionRegression(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.expected_results_file = (
            self.test_data_dir / "silence_detection_expected.json"
        )
    ...
    def _generate_test_wav_files(self):
        ...
        test_patterns = {
            "short_silence.wav": {
                ...
                "expected_silences": [{"start": 1.0, "duration": 0.5, "end": 1.5}],
            },
            "long_silence.wav": {
                ...
                "expected_silences": [{"start": 1.0, "duration": 1.5, "end": 2.5}],
            },
            ...
            "noisy_silence.wav": {
                ...
                "expected_silences": [{"start": 1.0, "duration": 1.0, "end": 2.0}],
            },
    ...
    def _generate_audio_pattern(self, pattern):
        ...
        for segment_type, duration in pattern:
            ...
            if segment_type == "speech":
                ...
                silence = np.random.normal(0, 0.001, samples)  # Much quieter noise
    ...
    def _load_expected_results(self):
        ...
        with open(self.expected_results_file) as f:
            return json.load(f)
    ...
    def test_long_silence_detection(self):
        ...
        max_silence = max(silence_events, key=lambda x: x["duration"])
        self.assertGreater(
            max_silence["duration"], 1200, "Long silence should be detected as >= 1.2s"
        )
    ...
    def test_multiple_silences_detection(self):
        ...
        self.assertGreaterEqual(
            len(silence_events), 2, "Should detect multiple silence periods"
        )
    ...
    def test_no_silence_detection(self):
        ...
        self.assertEqual(
            len(silence_events), 0, "Should not detect silence in continuous speech"
        )
    ...
    def test_different_presets(self):
        ...
        for preset, result in results.items():
            self.assertIn(
                "is_silent", result, f"Preset {preset} should return is_silent"
            )
            self.assertIsInstance(
                result["is_silent"],
                bool,
                f"Preset {preset} is_silent should be boolean",
            )
    ...
    def test_parameter_sensitivity(self):
        ...
        for min_len, result in results.items():
            self.assertIn(
                "is_silent",
                result,
                f"min_silence_len={min_len} should return is_silent",
            )
        ...
        for thresh, result in results.items():
            self.assertIn(
                "is_silent", result, f"silence_thresh={thresh} should return is_silent"
            )
